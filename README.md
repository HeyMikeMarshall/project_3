...
I ran progressively longer tests in succession. After running 1- and 12-hour tests, resource usage was found to be stable and the application appeared to run well. Having run the application for multiple-hour stints successfully, I began the first 7-day test. Between Saturday morning and Sunday night, the script collected just over 3 million records, which amounted to a bit over a gigabyte of data. The application appeared to be stable while running over multiple consecutive days, including morning and afternoon rush hours and by Wednesday I had collected over 15 million records. At this stage,it was apparent that I was collecting an immense amount of data, now I needed to start thinking about exactly what I was going to do with it all.

By the next weekend, the system at some point did encounter an error that locked up the service, but only after it had collected several hundred million data points from the Metro API. At first I thought of it as a success, but I soon encountered a substantial problem. Remote queries to the MongoDB would time out, and the latency too much to be acceptable. We were going to need additional horsepower to get this thing rolling.

In order to remedy this, I gave some thought to the project scope, and I decided if I was already treading in unfamiliar waters, I might as well just swim out deeper. I had 7 total Raspberry Pi 3b's available to me, and a Pi 4b on the way. Instead of just migrating the service to a 4b, which would have superior resources available to it, I figured, "let's just build a cluster".
